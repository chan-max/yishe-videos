...
class FFmpeg {
  ...
  async composeVideo(resources, outputPath, options = {}) {
    ...
    images.forEach((img) => {
      ...
      // 添加输入选项（修复：对于图像输入，FFmpeg 8.0+ 可能不支持 -loop 选项）
      // 尝试使用 -framerate 1 配合多次输入来实现循环效果
      // 对于静态图像，将输入帧率设为 1，然后在 filter 中使用 loop 来处理
      // 注意：不使用输入端的 -loop 选项
      ...
      let filter = buildScaleAndPositionFilter(`[${inputIndex}:v]`, img, width, height);
      
      // 设置帧率和像素格式
      // 对于单帧图像输入，需要使用 loop filter 来循环生成足够的帧数
      // loop filter 语法：loop=loop=N:size=S:start=START
      // 对于单帧输入，size=1，需要循环 totalFrames-1 次来得到 totalFrames 帧
      const totalFrames = Math.ceil(duration * fps);
      if (totalFrames > 1) {
        // 需要循环：loop=totalFrames-1 表示额外循环 totalFrames-1 次（总共 totalFrames 帧）
        filter += `,loop=loop=${totalFrames - 1}:size=1:start=0`;
      }
    ...
    // 计算最大时长（从所有视频输入中取最长时长）
    const maxDuration = Math.max(...videoInputs.map(v => v.duration || 0));
    if (maxDuration > 0) {
      // 明确限制输出时长为最大输入时长，防止死循环
